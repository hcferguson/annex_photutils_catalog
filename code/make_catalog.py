...
class PhotCat:
    ...
    def add_extra_columns(self,
        ...
        self.photometry_coadd.to_table(columns=self.my_photcolumns)
    ...
    def extra_kron_photometry(self,bands,kron_params=[4.,2.0,2.0],kron_name="kron4_2",overwrite=False):
        ...
        for b  in bands:
            ...
            self.photcats[b][f"{kron_name}_mag"] = -2.5*np.log10(
                    self.photcats[b][f"{kron_name}_flux"]) + self.ab_zpt
    ...
    def fluxfrac_radii(self,fluxfracs,bands,overwrite=False):
        for ff in fluxfracs:
